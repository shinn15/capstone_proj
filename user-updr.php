...
//display data
$sql="SELECT * FROM medrec_userd WHERE id='$id'";
$result=mysqli_query($mysqli,$sql);
$row=mysqli_fetch_assoc($result);
...
$sqli="SELECT * FROM medrec_personal WHERE id='$id'";
...
if(isset($_POST['submit_udat'])){
	...
	//check if age is = to number
 	if(!preg_match("/[0-9]/",$_POST['ua_u'])){
	  ...
	  $alert1="<script>alert('age must contain number!'); window.location.href='user-updr.php'</script>";
	  die($alert1);
...
// || = and
	...
	//display the error and redirect
	  $alert1="<script>alert('please enter legal age!'); window.location.href='user-updr.php'</script>";
	  die($alert1);
...
//check height and weight
	if(!preg_match("/[0-9]/",$_POST['us_h'],$_POST['us_w'])){
	 	...
	 	//display the error and redirect
	  $alert1="<script>alert('height and weight must contain number!'); window.location.href='user-updr.php'</script>";
	  die($alert1);
	...
	$sqli="UPDATE medrec_userd SET id='$id',time_ed='$oras',user_age='$userage',user_birthday='$userbd',user_height='$userh',user_weight='$userw',user_gend='$user_s',user_bloodt='$userbld' WHERE id='$id'";

	$result=mysqli_query($mysqli,$sqli);

	 $alert1="<script>alert('User Data Recorded Updated successfully'); window.location.href='process-userp.php'</script>";
...
if(isset($_POST['submit_pud'])){
    ...
    $sql2="SELECT * FROM medrec_personal WHERE id='$id'";
    ...
    if(@$chck==null){
    	...
    	$sqli="INSERT INTO medrec_personal (id,user_curr,user_vices,first_menstrual,lastin_for,last_menstrual,
														family_history,previous_ill,first_cvaccine,first_cdate,second_cvaccine,
														second_cdate,booster_cvac,booster_cdate)
											VALUES('$id','$usercu','$uvice_ot','$f_menst','$ls_menst','$lt_menst','$fam_il','$prev_il',
...
<head>
	...
	<title> User Medical Record Page</title>
