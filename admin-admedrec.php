...
if(isset($_SESSION["user_id"])){
		...
		$ussermail=$row['user_email'];//user email
...
echo("this record belong to ".$cur_rec);

//check if the user have already record
$sql2="SELECT * FROM medrec_userd WHERE email_user='$ussermail'";
$result2=mysqli_query($mysqli,$sql2);
$row2=mysqli_fetch_assoc($result2);

//@ to ignore error msg
if(@$row2['id']==$cur_id){
	$alert10="<script>alert('Already have Record! Please check the record to update'); window.location.href='admin-medrec.php'</script>";
	die($alert10);
...
if(isset($_POST['submit_ud'])){
	...
	//$user_rec=$cur_rec;
	$oras=date("Y-m-d");
	$usernamu=$_POST['name_u'];
	$usermaile=$_POST['us_email'];
	$userage=$_POST['ua_u'];
	$userbd=$_POST['u_btd'];
	$userh=$_POST['us_h'];
	$userw=$_POST['us_w'];
...
//check the bloodtype
 if($userbld==$_POST['blod_0']){
  ...
  die($alert1);
	...
	$sqli="INSERT INTO medrec_userd(time_ed,user_curr,user_namu,email_user,user_age,user_birthday,user_height,user_weight,user_gend,user_bloodt,prescript)
												VALUES('$oras','$usermaile','$usernamu','$usermaile','$userage','$userbd','$userh','$userw','$user_s','$userbld','$presc')";

	 $result=mysqli_query($mysqli,$sqli);

	  $alert1="<script>alert('User Data Recorded successfully'); window.location.href='admin-medrec.php'</script>";
...
if($uvice_ot != null){
	...
	$result3=mysqli_query($mysqli,$ssq);
	};

//insert in da databaae
$sqlii="INSERT INTO medrec_personal(user_curr,user_vices,first_menstrual,lastin_for,last_menstrual,
														family_history,previous_ill,first_cvaccine,first_cdate,second_cvaccine,
														second_cdate,booster_cvac,booster_cdate)
						VALUES('$usermaile','$uvice','$f_menst','$ls_menst','$lt_menst','$fam_il','$prev_il',
										'$f_vac','$f_vacd','$s_vac','$s_vacd','$b_vac','$b_vacd')";

$result2=mysqli_query($mysqli,$sqlii);

$alert1="<script>alert('User Personnal Medical Data Recorded successfully'); window.location.href='admin-medrec.php'</script>";
...
<div id="settings" class="tabcontent" style="display:block">
  ...
  <form method="post">
    ...
    <input type="Text" class="input_box" placeholder="User Full Name: " id="name_u" name="name_u" required>
    
    <label for="Text">User Email: </label>
    <input type="Text" class="input_box" placeholder="User Email: " id="us_email" name="us_email" required>

  	<label for="Text">Age: </label>
	<input type="Text" class="input_box" placeholder="User Age: " id="ua_u" name="ua_u" required>

	<label for="Text">Birthday: </label>
	<input type="Text" class="input_box" placeholder="User Birthday: " id="u_btd" name="u_btd" required>
